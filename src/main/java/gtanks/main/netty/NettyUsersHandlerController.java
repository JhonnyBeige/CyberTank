...
public class NettyUsersHandlerController {
    ...
    public void onMessageRecived(ChannelHandlerContext ctx, MessageEvent msg) {
        ...
        //check key first char if 0 then ignore encryption
        if(String.valueOf(message.charAt(0)).equals("0") && (this.sessions.get(channel).encryptionEnabled == -1 || this.sessions.get(channel).encryptionEnabled == 1)){
            
            this.sessions.get(channel).decryptProtocol(message, 0);
            return;
        }
        this.sessions.get(channel).decryptProtocol(decode(message));
    }
    ...
    public void authorizeSession(Session session, long userId) {
        if (this.authorizedSessions.containsKey(userId)) {
            ...
            if(this.sessions.get(activeSession.getChannel()) != null){
