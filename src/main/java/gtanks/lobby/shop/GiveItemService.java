...
public class GiveItemService implements MessageConsumer.ExternalMessageListener {
    ...
    private GiveItemService() {
        //kafkaTemplateService.getConsumer().addListener("give-item-request", this);
    }
    ...
    public void onReceive(String message) {
        ...
        } else if (fullItemId.equals("battlepass")) {
            ChallengesServices.instance().giveUserBattlePass(request.getUserId());
        } else if (fullItemId.equals("stars")) {
            ChallengesServices.instance().addStarsToUser(this.databaseManager.getUserById(request.getUserId()), request.getCount());
        ...
        } else if (fullItemId.equals("score")) {
            Optional.ofNullable(lobbysServices.getLobbyByUserId(request.getUserId()))
                    .ifPresentOrElse(lobbyManager -> lobbyManager.getLocalUser().addScore((int) request.count),
                            () -> {
                                User user = databaseManager.getUserById(request.getUserId());
                                user.addScore(request.count);
                                databaseManager.update(user);
                            });
