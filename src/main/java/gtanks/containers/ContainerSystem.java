    ...
    public String getUserContainersResponse(Long userId) {
        ...
        List<ListContainerResponse.ContainerInfo> containersInfo = userContainerRepository
            ...
            .map(container -> {
                ...
                ContainerItemInfo containerItemInfo = containers.get(containerId);
                
                // Print out the containerId and associated data
                System.out.println("Mapping UserContainer: containerId=" + containerId);

                ListContainerResponse.ContainerInfo info = new ListContainerResponse.ContainerInfo();
                info.setId(containerItemInfo.getClientId());
                info.setTitle(localization == Localization.EN ? containerItemInfo.getTitleEn() : containerItemInfo.getTitleRu());
                info.setDesc(localization == Localization.EN ? containerItemInfo.getDescEn() : containerItemInfo.getDescRu());
                info.setCount(container.getCount());

                // Debug output
                System.out.println("ContainerInfo: id=" + info.getId() + ", title=" + info.getTitle() + ", desc=" + info.getDesc() + ", count=" + info.getCount());
