...
public class TankKillModel
    ...
    public synchronized void damageTank(BattlefieldPlayerController controller, BattlefieldPlayerController damager,
        ...
        Integer module1res = controller.tank.getModule(0) != null ?
                controller.tank.getModule(0).getResistance(damager.tank.getWeaponInfo().getEntity().getType()) : 0;
        Integer module2res = controller.tank.getModule(1) != null ?
                controller.tank.getModule(1).getResistance(damager.tank.getWeaponInfo().getEntity().getType()) : 0;
        Integer module3res = controller.tank.getModule(2) != null ?
                controller.tank.getModule(2).getResistance(damager.tank.getWeaponInfo().getEntity().getType()) : 0;
        Integer resistance = module1res + module2res + module3res;
        if (controller.battle.battleInfo.battleFormat != 0 && controller.battle.battleInfo.battleFormat != 4) {
            resistance = 0;
        }
        ...
        DailyMissionsServices.instance().doDamage(damager.getUser(), new Double(damage).intValue());
    ...
    public synchronized void killTank(BattlefieldPlayerController controller, BattlefieldPlayerController killer) {
        ...
        if(!tank.isJuggernaut){
            controller.send(Type.BATTLE, "local_user_killed");
        }
        if (killer == null) {
            if(tank.isJuggernaut){
                this.bfModel.jgrModel.userKillJuggernaut(controller);
            }
            ...
            if (controller == killer) {
                ...
                DailyMissionsServices.instance().destroyTank(killer.getUser(), 1);
                if (this.bfModel.battleInfo.team) {
                    ...
                    if(this.battleInfo.battleType.equals("JGR")){
                        if(this.bfModel.jgrModel.isUserJuggernaut(controller)){
                            this.bfModel.jgrModel.userKillJuggernaut(killer);
                        }else if(this.bfModel.jgrModel.isUserJuggernaut(killer)){
                            this.bfModel.jgrModel.juggernautKillUser();
                        }
                    }
                    this.bfModel.statistics.changeStatistic(killer);
                    this.bfModel.statistics.changeStatistic(controller);
    ...
    public double fundMultiplier = 1.4;

    public double eventfundMultiplier = 3.0;

    public void addFund(double value) {
        this.battleFund += value * 10 * fundMultiplier * eventfundMultiplier;
