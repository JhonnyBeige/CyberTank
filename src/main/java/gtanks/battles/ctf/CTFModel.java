...
extends CaptureTheFlagAnticheatModel {
    ...
    public void attemptToTakeFlag(BattlefieldPlayerController taker, String flagTeamType) {
        ...
        if (taker.playerTeamType.equals(flagTeamType)) {
            ...
            if (enemyFlag.owner == taker) {
                ...
                if(this.bfModel.battleInfo.battleFormat != 4){
                    for (BattlefieldPlayerController previousOwner : enemyFlag.previousOwners){
                        int oppositePlayersCount = (taker.playerTeamType == "BLUE" ? this.bfModel.battleInfo.redPeople : this.bfModel.battleInfo.bluePeople);
                        if(previousOwner.parentLobby != null && previousOwner.battle != null && oppositePlayersCount > 0){
                            this.tanksServices.addScore(previousOwner.parentLobby, 5);
                            this.bfModel.statistics.changeStatistic(previousOwner);
                        }
                    }
                }
                DailyMissionsServices.instance().captureFlag(enemyFlag.owner.getUser(), 1);
                ...
                int scorePlayers = (taker.playerTeamType == "BLUE" ? this.bfModel.battleInfo.redPeople : this.bfModel.battleInfo.bluePeople);
                if(scorePlayers > 5){
                    scorePlayers = 5;
                }
                int score = scorePlayers * 15;
                if(this.bfModel.battleInfo.battleFormat != 4){
    ...
    public void returnFlag(BattlefieldPlayerController following, FlagServer flag) {
        ...
        if (following != null) {
            DailyMissionsServices.instance().returnFlag(following.getUser(), 1);
            flag.returnFlag();
        }
        flag.previousOwners.clear();
