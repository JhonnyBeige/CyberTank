...
public class TanksServices {
    ...
    public static int garageSales = 0;
    public static int microUpgradesSales = 40;

    public static TanksServices getInstance() {
        return instance;
    }

    public double xpMultiplier = 2;

    public void addScore(LobbyManager lobby, int score) {
        if(lobby.session.encryptionEnabled == 1){
            return;
        }
        boolean fall;
        if (lobby == null) {
        ...
        user.addScore(((Double)(score * xpMultiplier)).intValue());
        if(lobby.battle != null){
            lobby.battle.statistic.addScore(score);
            DailyMissionsServices.instance().gainScore(user, score);
        }
        ...
        if (increase || fall) {
            ...
            user.setNextScore(RankUtils.getRankByIndex((int)user.getRang()).max + 1);
